...
@router.get("/{session_id}/summary", response_model=TechnicalSummaryResponse)
async def get_interview_summary(
    ...
    try:
        ...
        try:
            update_response = supabase.table("interview_sessions").update({"session_status": "completed"}).eq("id", session_id).neq("session_status", "completed").execute()
            if update_response.data and len(update_response.data) > 0:
                logger.info(f"[SUMMARY] ✓ Session status updated to completed")
            else:
                logger.info(f"[SUMMARY] Session already completed")
